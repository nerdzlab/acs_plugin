...
struct TextMessageView: View {
        ...
        Group {
            if showUsername && messageModel.senderDisplayName != nil {
                Text(messageModel.senderDisplayName!)
                    .font(.caption)
                    .fontWeight(.bold)
                    .foregroundColor(Color(ChatStyleProvider.color.textPrimary))
            }
        }
    }
    ...
    var edited: some View {
        Group {
            if messageModel.editedOn != nil {
                Text("Edited")
                    .font(.caption)
                    .foregroundColor(Color(ChatStyleProvider.color.textDisabled))
            }
        }
    }
    ...
    private func getMessageBubbleBackground(messageModel: ChatMessageInfoModel) -> Color {
        guard messageModel.isLocalUser else {
            return Color(ChatStyleProvider.color.surfaceTertiary)
        }

        if messageModel.sendStatus == .failed {
            return Color(ChatStyleProvider.color.dangerPrimary).opacity(0.2)
        } else {
            return Color(ChatStyleProvider.color.primaryColorTint30)
        }
    }
}
